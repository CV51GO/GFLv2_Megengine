    ...
    def forward(self, inputs):
        ...
        for i in range(used_backbone_levels - 1, 0, -1):
            if 'scale_factor' in self.upsample_cfg:
                laterals[i - 1] += F.vision.interpolate(laterals[i],
                                                 **self.upsample_cfg)
            else:
                prev_shape = laterals[i - 1].shape[2:]
                laterals[i - 1] += F.vision.interpolate(
...
if __name__ == '__main__':
    ...
    input = (megengine.Tensor(np.random.randn(1, 256, 200, 304)),
            megengine.Tensor(np.random.randn(1, 512, 100, 152)),
            megengine.Tensor(np.random.randn(1, 1024, 50, 76)),
            megengine.Tensor(np.random.randn(1, 2048, 25, 38)))
